...
class App {
  ...
  boundOnTouchUp!: () => void;
  boundOnMouseEnter!: (e: MouseEvent) => void;
  ...
  lastHoverClientX: number | null = null;
  ...
  onMouseEnter(e: MouseEvent) {
    this.hoverScrollActive = true;
    this.lastHoverClientX = e.clientX;
  }

  onContainerMouseLeave() {
    this.onTouchUp();
    this.hoverScrollActive = false;
    this.lastHoverClientX = null;
  }

  /** Défilement au survol : uniquement le mouvement horizontal de la souris. */
  onMouseMoveHover(e: MouseEvent) {
    if (this.isDown || !this.hoverScrollActive || this.lastHoverClientX === null) return;
    const dx = e.clientX - this.lastHoverClientX;
    this.lastHoverClientX = e.clientX;
    this.scroll.target += dx * this.scrollSpeed * 0.025;
  ...
  onWheel(e: Event) {
    ...
    const dxRaw = wheelEvent.deltaX;
    const dxFromShift = wheelEvent.shiftKey ? dy : 0;
    const dx = dxRaw || dxFromShift;

    if (dx === 0) return;

    /* Ne pas bloquer le scroll page : n’agir que si le geste est surtout horizontal (trackpad envoie souvent un peu de deltaX avec un fort deltaY). */
    const absX = Math.abs(dxRaw);
    const absY = Math.abs(dy);
    const primarilyHorizontal =
      wheelEvent.shiftKey || absX > absY * 0.55 + 2;

    if (!primarilyHorizontal) return;

    e.preventDefault();
    this.scroll.target += dx * this.scrollSpeed * 0.02;
    this.onCheckDebounce();
  }
  ...
  addEventListeners() {
    ...
    this.boundOnMouseLeave = this.onContainerMouseLeave.bind(this);
